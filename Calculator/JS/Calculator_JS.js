...
const Calculator = {
    ...
    First_Operand: null,
    //this checks whether or rnot the second operrand has been input
    Wait_Second_Operand: false,
    //this will hold the operator, we set it to null for now
    operator: null,
};

//this modifies values each timea button is clicked
...
function Input_Decimal(dot) {
    ...
    //we arre sahying that if the Display_Value does not contain a decimal point we want to add a decimal point.
...
function Handle_Operator(Next_Operator) {
    ...
    //checks if an operator already exists and if Wait_Second_Operand is trrue, then update the operator and exits frorm the function
    ...
    if (First_Operand == null) {
        ...
        //if operator exists, property lookup is perforrmed for the operator in the Perform_Calculation object and the function
...
keys.addEventListener('click', (event) => {
    ...
    const { target } = event;
    //if the element that was clicked onn is not a button, exit the function
